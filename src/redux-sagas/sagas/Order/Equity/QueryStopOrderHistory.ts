...
import { ORDER_QUERY_STOP_ORDER_HISTORY } from 'redux-sagas/actions';
...
function* doQueryStopOrderHistory(request: IRequest<IObject>) {
  try {
    ...
    if (config.usingNewKisCore) {
      request.payload.code = store.getState().currentSymbol?.s!;
    } else {
...
export default function* watchQueryStopOrderHistory() {
  yield takeEvery(ORDER_QUERY_STOP_ORDER_HISTORY, doQueryStopOrderHistory);
}
