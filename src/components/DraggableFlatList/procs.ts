...
export const getTranslate = proc((isHovering, currentIndex, activeIndex, isAfterHoverMid, activeCellSize) =>
  cond(and(isHovering, neq(currentIndex, activeIndex)), cond(isAfterHoverMid, activeCellSize, 0), 0)
);

export const getCellStart = proc((isAfterActive, size, offset, activeCellSize, scrollOffset) =>
  sub(cond(isAfterActive, sub(add(offset, size), activeCellSize), offset), scrollOffset)
);

export const getOnChangeTranslate = proc(
  (translate, isAfterActive, initialized, hoverScrollSnapshot, scrollOffset, toValue, isPressedIn) =>
    block([
      cond(or(not(isAfterActive), initialized), [set(hoverScrollSnapshot, scrollOffset)], set(initialized, 1)),
