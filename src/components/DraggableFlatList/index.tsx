...
import { Platform, StyleSheet, VirtualizedListProps, findNodeHandle, ViewStyle } from 'react-native';
import {
  PanGestureHandler,
  TapGestureHandler,
...
type Props<T> = Modify<
  VirtualizedListProps<T>,
  {
    ...
    renderItem: (params: { item: T; index: number; drag: () => void; isActive: boolean }) => JSX.Element;
...
class DraggableFlatList<T> extends React.Component<Props<T>, State> {
  ...
  onPanGestureEvent = event([
    {
      nativeEvent: ({ x, y }) =>
        block([
          cond(and(this.isHovering, eq(this.panGestureState, GestureState.ACTIVE), not(this.disabled)), [
  ...
  hoverComponentOpacity = and(this.isHovering, neq(this.panGestureState, GestureState.CANCELLED));
    ...
    return (
      <TapGestureHandler ref={this.tapGestureHandlerRef} onHandlerStateChange={this.onContainerTapStateChange}>
        <Animated.View style={styles.flex}>
          ...
          >
            <Animated.View style={styles.flex} ref={this.containerRef} onLayout={this.onContainerLayout}>
              <AnimatedFlatList
                {...this.props}
...
type RowItemProps = {
  drag: (hoverComponent: JSX.Element, itemKey: string) => void;
